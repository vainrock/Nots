...
@Component({
  ...
  template: `
    <div *ngIf="note"
         ...
         class="w-full h-[90vh] mx-auto mt-4 [perspective:2500px] flex">
      ...
      <div [class.fold-3d]="isClosing" [class.unfold-3d]="!isClosing"
        ...
        <div class="absolute inset-0 bg-gray-900 rounded-2xl border border-violet-500/30 flex flex-col items-center justify-center [backface-visibility:hidden] [transform:rotateY(180deg)] shadow-[0_0_50px_rgba(139,92,246,0.15)] overflow-hidden">
          ...
          <div class="w-24 h-24 border border-violet-500/50 rounded-full flex flex-col items-center justify-center mb-6 shadow-[inset_0_0_20px_rgba(139,92,246,0.2)] z-10">
            <span class="text-violet-400 font-mono text-sm tracking-widest uppercase opacity-70">Sector</span>
            <span class="text-violet-300 font-black text-2xl font-mono">{{ note.id }}</span>
          </div>

          <h3 class="font-mono text-gray-500 text-lg tracking-[0.3em] uppercase text-center border-b border-gray-800 pb-2 z-10">Encrypted_Data</h3>
          <span class="mt-4 text-xs text-pink-500 font-mono tracking-widest animate-pulse z-10">> UPDATING_RECORD...</span>
        </div>
